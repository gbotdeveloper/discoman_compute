...
When an app is set to run on your machine, GBot queues each request and this
program picks it up, runs your script with your Python, and sends the result
back. Nothing runs here unless someone uses your app.

Your account is never stored on disk — only a revocable per-machine token, at
`~/.discoman/credentials.json`.
